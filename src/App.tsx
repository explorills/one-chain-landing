          ...
          <div className="flex items-center justify-between py-3 sm:py-4 gap-4">
            ...
            <a href="/" className="flex items-center gap-[11px] shrink-0">
              <img
                src="/logo.png"
                alt="ONE Chain"
                className="w-[50px] h-[50px] sm:w-[58px] sm:h-[58px] object-contain"
                width="58"
                height="58"
                loading="eager"
              />
              <div className="flex flex-col gap-1.5">
                <p className="text-[22px] sm:text-[26px] font-bold tracking-tight leading-none">
...
function HeroSection() {
  return (
    <section className="relative min-h-[100svh] flex items-center overflow-hidden pt-20 sm:pt-16">
      <DataStream count={20} />
      ...
      <div className="absolute inset-0 lg:relative lg:inset-auto lg:flex-1 flex items-center justify-center pointer-events-none lg:pointer-events-auto">
        <div className="w-full max-w-[500px] lg:max-w-[600px] opacity-20 lg:opacity-100">
          <Globe />
        </div>
      </div>

      <div className="absolute inset-0 lg:relative lg:inset-auto max-w-7xl mx-auto px-4 sm:px-6 w-full flex items-center">
        <div className="grid lg:grid-cols-2 gap-6 w-full items-center">
          ...
          >
            <motion.div variants={fadeUp}>
              <span
                className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-medium bg-primary/10 text-primary border border-primary/20"
            ...
            <motion.h1
              variants={fadeUp}
              className="text-[32px] sm:text-5xl lg:text-6xl font-bold tracking-tight leading-[1.1]"
            >
              The Economic
              <br />
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-2.5 pt-1">
              <a
                href="#network"
                className="group inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-xl bg-gradient-to-r from-primary to-accent text-white hover:shadow-[0_0_30px_oklch(0.65_0.12_220_/_0.35)] transition-all duration-300"
              >
                Explore Network
              ...
              <a
                ...
                className="group inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-xl border border-border/60 hover:border-primary/40 hover:bg-card/50 transition-all duration-200"
              >
                View Source
            ...
            <motion.div variants={fadeUp} className="flex gap-5 sm:gap-6 pt-2">
              {[
                ...
                <div key={spec.label} className="text-center">
                  <div className="text-base sm:text-lg font-semibold text-foreground" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                    {spec.value}
                  </div>
                  <div className="text-[11px] text-muted-foreground mt-0.5">{spec.label}</div>
                </div>
              ))}
            </motion.div>
          </motion.div>
...
function NetworkSection() {
  ...
  return (
    <section id="network" className="relative py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <motion.div
          variants={stagger}
          initial="hidden"
        ...
        >
          <motion.span
            variants={fadeUp}
            className="inline-block text-[11px] font-medium uppercase tracking-[0.25em] text-primary mb-3"
            style={{ fontFamily: "'JetBrains Mono', monospace" }}
          >
            Network Statistics
          </motion.span>
        ...
        >
          {stats.map((stat) => (
            <motion.div
              key={stat.label}
              variants={scaleIn}
              className="group relative p-4 sm:p-5 rounded-xl bg-card/40 border border-border/40 backdrop-blur-sm hover:border-primary/30 hover:bg-card/60 transition-all duration-300"
            >
              <div className="absolute top-3 right-3">
                <stat.icon weight="duotone" className="w-4 h-4 sm:w-5 sm:h-5 text-primary/40 group-hover:text-primary/60 transition-colors" />
              </div>
...
function ArchitectureSection() {
  ...
  return (
    <section id="architecture" className="relative py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <motion.div
          variants={stagger}
          initial="hidden"
        ...
        >
          ...
          <motion.p variants={fadeUp} className="text-sm text-muted-foreground mt-3 max-w-xl mx-auto">
        ...
        >
          {layers.map((layer, i) => (
            <motion.div
              key={layer.title}
              variants={fadeUp}
              className="group relative flex items-center gap-3 sm:gap-4 p-3.5 sm:p-4 rounded-xl border border-border/30 bg-card/30 backdrop-blur-sm hover:border-primary/30 hover:bg-card/50 transition-all duration-300"
            >
              <div
                className="shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center text-xs sm:text-sm font-bold"
                style={{ background: `${layer.color}15`, color: layer.color, fontFamily: "'JetBrains Mono', monospace" }}
              >
                L{layers.length - i}
              </div>
              <div className="shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center hidden sm:flex" style={{ background: `${layer.color}15` }}>
                <layer.icon weight="duotone" className="w-5 h-5" style={{ color: layer.color }} />
              </div>
              <div className="min-w-0">
                <h3 className="text-sm sm:text-base font-semibold mb-0.5">{layer.title}</h3>
                <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{layer.desc}</p>
              </div>
              {i < layers.length - 1 && (
                <div className="absolute left-[2.05rem] sm:left-[2.25rem] top-full w-px h-2.5 z-10" style={{ background: `linear-gradient(to bottom, ${layer.color}40, transparent)` }} />
              )}
            </motion.div>
          ))}
        </motion.div>
...
function NodesSection() {
  ...
  return (
    <section id="nodes" className="relative py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <motion.div
          variants={stagger}
          initial="hidden"
        ...
        >
          {features.map((feature) => (
            <motion.div
              key={feature.title}
              variants={scaleIn}
              className="group p-4 sm:p-5 rounded-xl bg-card/30 border border-border/30 backdrop-blur-sm hover:border-primary/25 hover:bg-card/50 transition-all duration-300"
            >
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-primary/10 flex items-center justify-center mb-3 group-hover:bg-primary/15 transition-colors">
                <feature.icon weight="duotone" className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-primary" />
              </div>
              <h3 className="text-sm sm:text-base font-semibold mb-1.5">{feature.title}</h3>
...
function ConsensusSection() {
  return (
    <section id="consensus" className="relative py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          >
            <div className="p-5 sm:p-6 rounded-xl bg-card/30 border border-border/30 backdrop-blur-sm">
                  ...
                  <div key={validator} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs sm:text-sm font-medium">{validator}</span>
                        {i === 0 && (
                          <span className="text-[9px] sm:text-[10px] px-1.5 py-0.5 rounded-full bg-primary/15 text-primary font-medium" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                            PRODUCING
                          </span>
                        )}
                      </div>
              ...
              <div className="mt-4 p-3 rounded-lg bg-muted/30 border border-border/30">
                <div className="flex items-center gap-2.5">
                  <Cube weight="duotone" className="w-4 h-4 text-primary shrink-0" />
                  <div>
                    <div className="text-xs sm:text-sm font-medium">Round-Robin Block Production</div>
                    <div className="text-[11px] text-muted-foreground mt-0.5">Validators take turns every 3 seconds. Deterministic.</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <motion.h2 variants={fadeUp} className="text-2xl sm:text-4xl font-bold tracking-tight mb-4">
              Proof of Authority
            </motion.h2>
            <motion.div variants={fadeUp} className="space-y-3 text-xs sm:text-sm text-muted-foreground leading-relaxed">
              <p>
            ...
            <motion.div variants={fadeUp} className="grid grid-cols-2 gap-3 mt-5">
              {[
                ...
                <div key={item.label} className="p-2.5 rounded-lg bg-card/30 border border-border/30">
                  <div className="text-xs sm:text-sm font-semibold text-foreground" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{item.value}</div>
...
function RoadmapSection() {
  ...
  return (
    <section id="roadmap" className="relative py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <motion.div
          variants={stagger}
          initial="hidden"
        ...
        >
          {phases.map((phase) => (
            <motion.div
              key={phase.phase}
              variants={fadeUp}
              className={`group relative p-4 sm:p-5 rounded-xl border backdrop-blur-sm transition-all duration-300 ${
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3">
                <span
                  className={`text-[11px] font-bold uppercase tracking-wider ${phase.status === 'active' ? 'text-primary' : 'text-muted-foreground'}`}
                  style={{ fontFamily: "'JetBrains Mono', monospace" }}
                >
                  {phase.phase}
                </span>
                {phase.status === 'active' && (
                  <span className="flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded-full bg-primary/15 text-primary">
                    <span className="w-1 h-1 rounded-full bg-primary animate-pulse" />
                    Active
                  </span>
                )}
              </div>
              <h3 className="text-sm sm:text-base font-semibold mb-3">{phase.title}</h3>
  ...
  return (
    <>
          ...
          <div className="py-3">
            <div className="flex items-center justify-between gap-4">
              <div className="text-xs whitespace-nowrap">
                ...
                <span className="text-muted-foreground"> &copy; {new Date().getFullYear()} All rights reserved</span>
              </div>
      ...
      <motion.button
        ...
        className="fixed bottom-20 right-6 sm:right-8 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/25 flex items-center justify-center transition-all duration-300 z-50 border border-primary/50"
